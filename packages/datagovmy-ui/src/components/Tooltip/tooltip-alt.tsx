...
const TooltipContent: ForwardRefExoticComponent<TooltipContentProps> = forwardRef(
  ({ className, asChild, children, container, forceMount, ...props }, ref) => (
    <Portal container={container} forceMount={forceMount}>
      <Content ref={ref} className={clx(tooltip_content_cva, className)} {...props}>
        {children}
        <TooltipArrow />
      </Content>
    </Portal>
...
const TooltipArrow: ForwardRefExoticComponent<ComponentProps<typeof Arrow>> = forwardRef(
  ({ className, ...props }, ref) => (
    <Arrow ref={ref} className={clx("fill-bg-black-900", className)} {...props} />
  )
);
