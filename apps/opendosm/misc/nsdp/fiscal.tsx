...
type FiscalTabProps = {
  fiscal: Record<string, WithData<Record<string, Record<string, number[]>>> & { x_freq: string }>;
  chartColor: [string, string];
};

const NSDPFiscal: FunctionComponent<FiscalTabProps> = ({ fiscal, chartColor }) => {
  const { t } = useTranslation(["nsdp"]);
  const scrollRef = useRef<Record<string, HTMLElement | null>>({});
  const { size } = useContext(WindowContext);
  const data = Object.entries(fiscal) || [];
          ...
          <>
            {categories.map(([category], index) => (
              <li key={`${category}`} title={category}>
                ...
                >
                  {t(`section_fiscal.${category}.title`)}
                </Button>
              </li>
            ))}
          </>
      ...
      {data.map(([section, dt]) => (
        <Fragment key={section}>
          <SliderProvider>
            {play => (
              <Section
                title={t(`section_fiscal.${section}.title`)}
                date={dt.data_as_of}
                ref={ref => {
              ...
              >
                <NSDPTimeseriesSection
                  baseTranslation={`section_fiscal.${section}`}
                  datum={dt}
                  play={play}
