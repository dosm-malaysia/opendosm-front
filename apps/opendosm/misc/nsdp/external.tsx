...
type ExternalTabProps = {
  external: Record<string, WithData<Record<string, Record<string, number[]>>> & { x_freq: string }>;
  chartColor: [string, string];
};

const NSDPExternal: FunctionComponent<ExternalTabProps> = ({ external, chartColor }) => {
  const { t } = useTranslation(["nsdp"]);
  const scrollRef = useRef<Record<string, HTMLElement | null>>({});
  const { size } = useContext(WindowContext);
  const data = Object.entries(external) || [];
          ...
          <>
            {categories.map(([category], index) => (
              <li key={`${category}`} title={category}>
                ...
                >
                  {t(`section_external.${category}.title`)}
                </Button>
              </li>
            ))}
          </>
      ...
      {data.map(([section, dt]) => (
        <Fragment key={section}>
          <SliderProvider>
            {play => (
              <Section
                title={t(`section_external.${section}.title`)}
                date={dt.data_as_of}
                ref={ref => {
              ...
              >
                <NSDPTimeseriesSection
                  baseTranslation={`section_external.${section}`}
                  datum={dt}
                  play={play}
